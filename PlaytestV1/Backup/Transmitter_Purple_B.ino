/*
 * Wobble Unified - Transmitter BACKUP (Purple Rocker B)
 * Adafruit QT Py ESP32-S3
 *
 * BACKUP BOARD — identical to Transmitter_Purple.ino except for sensor port.
 * Flash this onto the spare Purple board. Keep the primary board powered if
 * possible; Python listens on both ports simultaneously.
 *
 * This board:
 * - Broadcasts BLE beacon for proximity detection (unique name "QTPy_ESP32_B" — backup set only)
 * - Sends LSM6DSOX sensor data via OSC to port 8105
 *
...
const int oscSensorPort = 8105;               // Backup port for LSM6DSOX data → Python
const int oscLocalPort = 9105;                // Local port for OSC
...
// ===== BLE CONFIGURATION =====
// IMPORTANT: beacon name must stay "QTPy_ESP32" so receivers can detect this transmitter
#define DEVICE_NAME "QTPy_ESP32_B"
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("Wobble Unified - Transmitter BACKUP (Purple B)");
  ...
  // Initialize BLE — MUST broadcast as "QTPy_ESP32" for receivers to detect it
  ...
  Serial.println("  - Sensor data: Port 8105 → Python Unified Processor");

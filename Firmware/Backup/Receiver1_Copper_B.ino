/*
 * Wobble Unified - Receiver 1 BACKUP (Copper Rocker B)
 * Adafruit QT Py ESP32-S3
 *
 * BACKUP BOARD — identical to Receiver1_Copper.ino except for port numbers.
 * Flash this onto the spare Copper board. Keep the primary board powered if
 * possible; Python listens on both ports simultaneously.
 *
 * This board:
 * - Scans for BLE beacon and detects proximity
 * - Sends proximity events to port 8101 (/proximity/distance)
 * - Sends LSM6DSOX sensor data to port 8104
 *
...
const int oscProximityPort = 8101;  // Backup port for /proximity/distance messages
const int oscSensorPort = 8104;      // Backup port for LSM6DSOX sensor data
const int oscLocalPort = 9100;       // Local port for OSC
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("Wobble Unified - Receiver 1 BACKUP (Copper B)");
  ...
  BLEDevice::init("QTPy_Receiver1_Copper_B");
  ...
  Serial.println("  Port 8101: /proximity/distance (distance + in_range)");
  Serial.println("  Port 8104: Sensor bundle (25Hz)");
